...
// 상태코드를 controller에서 지정해서 내려줄 수 있게 해주는 @RestController
@RestController
public class DiaryController {
    ...
    // GET -> 조회할 때, POST -> 저장할 때
    // @RequestParam -> 요청을 보낼 때 넣어주는 파라미터
    // @RequestBody -> POST요청은 body에다가 데이터를 넣어서 전송할 수 있다.
    ...
    List<Diary> readDiaries(@RequestParam @DateTimeFormat(iso= DateTimeFormat.ISO.DATE)
                            @ApiParam(value = "조회할 기간의 첫번째 날입니다.", example = "2020-02-02")
                            LocalDate startDate,
